...
var artwork_texture = function(url, material, scene, onLoaded, targetAspect){
	...
	// On touch devices, use the pre-generated mobile artwork when available.
	// The mobile copies are resized to a 1600px longest edge, preserving aspect ratio.
	// Keep blob/data URLs untouched so the built-in preview mode continues to work.
	var mobileUrl = url;
	if (typeof isTouchDevice !== 'undefined' && isTouchDevice && !/^blob:|^data:/i.test(url)) {
		mobileUrl = url.replace(/\\/(archive|ken|of)\\//i, '/$1/mobile/');
	}
	...
	img.onload = function(){
		...
		var planeAspect = Number(targetAspect) > 0 ? Number(targetAspect) : sourceAspect;

		// On touch devices, keep the artwork's original aspect ratio inside the
		// existing 3D artwork plane. The canvas is made to match the plane ratio,
		// while drawImage() uses a "contain" calculation, so portrait/landscape
		// images are never stretched. Transparent padding is used when the two
		// ratios differ.
		var texW, texH;
		if (planeAspect >= 1) {
			texW = Math.max(1, Math.round(cap));
			texH = Math.max(1, Math.round(cap / planeAspect));
		} else {
			texH = Math.max(1, Math.round(cap));
			texW = Math.max(1, Math.round(cap * planeAspect));
		}

		var tex = new BABYLON.DynamicTexture(url, {width: texW, height: texH}, scene, true);
		var ctx = tex.getContext();

		// Fit the source image inside the texture without cropping or stretching.
		var drawW, drawH;
		if (sourceAspect > planeAspect) {
			drawW = texW;
			drawH = Math.max(1, Math.round(texW / sourceAspect));
		} else {
			drawH = texH;
			drawW = Math.max(1, Math.round(texH * sourceAspect));
		}
		var drawX = Math.round((texW - drawW) / 2);
		var drawY = Math.round((texH - drawH) / 2);
...
function populate_template(config_file, room_name,scene){
	...
	for (var item of dict_items){
		...
		// The 3D plane already uses the artwork's configured physical size.
		// Pass its aspect ratio to the mobile texture builder so the source image
		// is contained inside the plane instead of being stretched.
		var artworkAspect = Number(gallery[item]["width"]) / Number(gallery[item]["height"]);
	...
	scene.meshes.map((mesh) => {
		...
		if (regul_exp_door.test(mesh.name)){
			if (renamed_doors >= max_doors){ //hide template doors not registered in the room config
